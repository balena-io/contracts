...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp balenalib/{{arch.sw.slug}}-{{sw.os.slug}}-{{sw.stack.slug}}:latest python your-daemon-or-script.py
```
