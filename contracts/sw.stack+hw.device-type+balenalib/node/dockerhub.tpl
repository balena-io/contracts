...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/app -w /usr/src/app balenalib/{{hw.device-type.slug}}-{{sw.os.slug}}-{{sw.stack.slug}}:latest node your-daemon-or-script.js
```
